...
- For `progress_saved`, failure, an open batch, or `backlog_growing`, record the organization cycle in `downstream_stages` as `not_run`; Dream is an internal second phase of this same local owner and is not a separate descendant. Do not launch the organization cycle, invoke a second Sleep child, or run a status-helper retry.
