...
9. Run the organization content-hash checkpoint. Use content hashes for duplicate analysis across `main`, imports, prior accepted uploads, and current proposals. Duplicate entry ids remain a visible maintenance finding; the snapshot upgrader assigns deterministic legacy ids so both bytes are preserved, and no duplicate may silently collapse into one card.
