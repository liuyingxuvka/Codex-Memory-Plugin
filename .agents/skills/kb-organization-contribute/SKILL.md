...
that other local KBs may use directly from a synchronized snapshot; organization
acceptance does not override each machine's local retrieval weighting or Sleep judgment.
...
For the organization composite task or an explicit diagnostic, intake, planning, or proposal-only output is incomplete. The organization cycle invokes the native contribution owner exactly once after organization maintenance and embeds its immutable receipt in the organization cycle receipt v3. A direct diagnostic may use `python scripts/run_kb_automation.py --skill kb-organization-contribute --json`, but it never creates an independent schedule. A settings-gated no-op counts only when the native gate receipt proves it terminal.
