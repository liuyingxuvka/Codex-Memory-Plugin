...
- [x] 8.8 Replace task-time organization lazy download/adoption with local foreign-snapshot retrieval and `organization_use` feedback; remove download-only exchange blocking, auto-install only on card presence, and user adoption UI side effects while preserving explicit local Sleep authority.
- [x] 8.9 Add one local maintenance cycle facade with `fresh_cycle` and `resume_sleep` modes, Sleep-first/Dream-second phase ordering, a shared lease, sealed Dream handoffs for the next Sleep batch, Sleep-only canonical publication, exact-once phase receipts, and cleanup/timeout isolation.
...
- [x] 8.15 Make local and organization cycle receipts immutable, idempotent, lease-bound, and child-receipt-referencing; enforce Sleep-first/Dream-second and one pinned organization sync context without mutating child receipts.
- [x] 8.16 Add deterministic duplicate/legacy-card upgrade handling, snapshot-token feedback resolution, and regression coverage for malformed bundles, duplicate ids, missing snapshots, and revision mismatch.
...
- [x] 9.7 Perform the explicit KB postflight, record mistake-first reusable lessons or route/card weaknesses as structured observations, update FlowGuard adoption evidence, and report any deliberately scoped release gaps without marking the OpenSpec change complete.
...
- [x] 9.10 Run the transactional installer and install check, then verify source-to-consumer projection, local repository parity, and local Git identity without touching unrelated peer paths.
- [x] 9.11 Run the single final readiness owner after all affected children are current, consume its terminal receipt in OpenSpec and FlowGuard closure, and keep any unresolved authority or peer-write blocker visible.
- [x] 9.12 Re-run the frozen FlowGuard model/test alignment and TestMesh after the repair tasks, update adoption evidence, and close only when source, install, repository, and Git identities are separately current.
