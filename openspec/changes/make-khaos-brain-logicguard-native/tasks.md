...
- [ ] 9.10 Run the transactional installer and install check, then verify source-to-consumer projection, local repository parity, and local Git identity without touching unrelated peer paths.
- [ ] 9.11 Run the single final readiness owner after all affected children are current, consume its terminal receipt in OpenSpec and FlowGuard closure, and keep any unresolved authority or peer-write blocker visible.
- [ ] 9.12 Re-run the frozen FlowGuard model/test alignment and TestMesh after the repair tasks, update adoption evidence, and close only when source, install, repository, and Git identities are separately current.
...
- [ ] 10.11 Make terminal Model-Test Alignment and TestMesh consume the frozen readiness evidence graph and JUnit/model receipts; planning-only `frozen_not_run`, empty `current_runs`, missing children, and stale identities must block readiness.
...
- [ ] 10.13 Freeze the final source/tool/owner inventory; run affected native checks and the single aggregate readiness owner, install/check the consumer projection, execute current local and organization cycle acceptance, perform KB postflight, then verify source/install/repository/Git/GitHub release identities separately.
